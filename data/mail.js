...
function _defaultComposeState(replyMail) {
    return {
...
function _groupSelectedByFolder() {
    const byFolder = {};
...
function openMailDetail(idx) {
    ...
    if (mail.read === false) { // コピーではなく元のオブジェクトを更新
    ...
    if (actionsEl) {
        if (currentMailFolder === 'draft') {
            ...
            `;
        } else if (currentMailFolder === 'sent' || currentMailFolder === 'favorites') {
            actionsEl.innerHTML = `
                <button class="mailbox-stamp-btn" onclick="trashCurrentMail()"><img src="status/mail/dustbox.png" class="stamp-icon-img"></button>
            `;
    ...
    if (mail.designImg) {
        const ff      = FONT_MAP[mail.fontFamily] || FONT_MAP.gothic;
...
// 一括：あとで返す（降順処理でインデックスずれ防止）
function bulkActionLater() {
    const byFolder = _groupSelectedByFolder();
...
let composeState = _defaultComposeState(null);
...
function doOpenComposeView(replyMail) {
    composeState = _defaultComposeState(replyMail);
    setMailSearchBarVisible(false);
    document.getElementById('mailboxListView').style.display = 'none';
    document.getElementById('mailboxDetailView').style.display = 'none';
    document.getElementById('mailboxComposeView').style.display = 'flex';
...
function renderComposeStep3() {
    ...
    if (bodyEl) { // drag-drop を無効化（画像の二重表示バグ防止）
...
function applyComposeBodyStyle() {
    const body = document.getElementById('composeBody');
    if (!body) return;
    const ff = FONT_MAP[composeState.fontFamily] || FONT_MAP.gothic;
...
function _markNotifTypesRead(types) {
...
function markCurrentNotifCategoryRead() {
    ...
    _markNotifTypesRead(getNotifFilterTypes(currentNotifFilter));
}
...
function getNotifActionText(n) {
    const name = `<strong>${escapeHtml(n.fromName)}</strong>`;
    switch (n.type) {
...
function renderNotifList() {
    ...
    el.innerHTML = sorted.map(n => {
        const isImg = typeof n.fromAvatar === 'string' && n.fromAvatar.includes('/');
...
function updateNotifBadge() {
    ...
    let likeCnt = 0, commentCnt = 0, friendCnt = 0, saisenCnt = 0, newsCnt = 0;
    notifs.forEach(n => {
        if (n.read) return;
        if (n.type === 'like' || n.type === 'stamp') likeCnt++;
        else if (n.type === 'comment' || n.type === 'reply') commentCnt++;
        else if (n.type === 'friend_build_house' || n.type === 'friend_build_town' || n.type === 'friend_request') friendCnt++;
        else if (n.type === 'saisen') saisenCnt++;
        else if (n.type === 'news') newsCnt++;
    });
